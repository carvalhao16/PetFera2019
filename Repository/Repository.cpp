...
int repository::callbackAnimal(void *NotUsed, int argc, char **argv, char **azColName){
    int i;
    list<animal_obj> animais_objs;
    animal_obj* animal ;
//    for(i = 0; i<argc; i++) {
//       printf("%d %s = %s\n",i, azColName[i], argv[i] ? argv[i] : "NULL");
//      if(azColName[i] == "ID"){
//         animal->id = argv[i] ;
//      }
//      else if(azColName[i]=="CLASSE"){
//          animal->classe = argv[i] ;
//      }
//      else if(azColName[i]=="NOME_ANIMAL"){
//          animal->nome_animal = argv[i] ;
//      }
//      else if(azColName[i]=="NOME_CIENTIFICO"){
//          animal->nome_cientifico = argv[i] ;
//      }
//      else if(azColName[i]=="SEXO"){
//          animal->sexo = argv[i] ;
//      }
//      else if(azColName[i]=="TAMANHO"){
//          animal->tamanho = argv[i] ;
//      }
//      else if(azColName[i]=="DIETA"){
//         animal->dieta = argv[i] ;
//      }
//      else if(azColName[i]=="VETERINARIO"){
//         animal->veterinario = argv[i] ;
//      }
//      else if(azColName[i]=="TRATADOR"){
//          animal->tratador = argv[i] ;
//      }
//      else if(azColName[i]=="NOME_DE_BATISMO"){
//          animal->nome_batismo = argv[i] ;
//      }
//      if(i==0){}
//      else if((i%10 == 0)){
...
void repository::select_init_animal(){
    sql = "SELECT * from ANIMAL";
    rc = sqlite3_exec(db, sql, callbackAnimal, 0, &zErrMsg);
   if( rc != SQLITE_OK ){
      cout<< "SQL error: "<< zErrMsg<<endl;
      sqlite3_free(zErrMsg);
   } 
